    ...
    for (int iter = 0; iter < max_iters; iter++) {
        for (uintV u = first_vertex; u <= final_vertex; u++) {
            ...
            for (uintE i = 0; i < out_degree; i++) {
                ...
                lock.lock();
                {
                    // In principle, v could be any vertex, so any thread could write to the same pr_next[v] at any
                    // time => need lock.
...
int main(int argc, char *argv[]) {
    ...
    options.add_options(
            "",
            {
                    ...
                    // TODO: VERY IMPORTANT. Add this back in,         "/scratch/input_graphs/roadNet-CA")},
